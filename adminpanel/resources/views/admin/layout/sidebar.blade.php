    ...
    <aside class="app-sidebar sidebar-scroll">
        ...
        <div class="main-sidemenu">
                ...
                <div class="dropdown user-pro-body text-center">
                    <div class="user-pic">
                        @php
                            ...
                            $canManageNewsPosts = $admin?->hasModuleAccess('post', 'view');
                            $canAddNewsPosts = $admin?->hasModuleAccess('post', 'add');
                            $canEditNewsPosts = $admin?->hasModuleAccess('post', 'edit');
                            $canDeleteNewsPosts = $admin?->hasModuleAccess('post', 'delete');
                            $canUseNewsPosts = $canManageNewsPosts || $canAddNewsPosts || $canEditNewsPosts || $canDeleteNewsPosts;
                            $canSeeAllNewsPosts = $admin?->type === 'superadmin';
            ...
            <ul class="side-menu">
                ...
                @endif
                @if ($canUseNewsPosts)
                <li class="slide {{ request()->routeIs('posts*') ? 'is-expanded' : '' }}">
                    <a class="side-menu__item" data-bs-toggle="slide" href="javascript:void(0)">
                        ...
                        <span class="side-menu__label">News Post</span><i class="angle fe fe-chevron-right"></i>
                    </a>
                    <ul class="slide-menu">
                        <li><a href="{{ route('posts.mine') }}" class="slide-item {{ request()->routeIs('posts.mine') ? 'active' : '' }}">My Posts</a></li>
                        @if ($canEditNewsPosts)
                            <li><a href="{{ route('posts.review') }}" class="slide-item {{ request()->routeIs('posts.review') ? 'active' : '' }}">Review Queue</a></li>
                        @endif
                        @if ($canSeeAllNewsPosts)
                            <li><a href="{{ route('posts') }}" class="slide-item {{ request()->routeIs('posts') ? 'active' : '' }}">All News</a></li>
                        @endif
                    </ul>
                </li>
                @endif
